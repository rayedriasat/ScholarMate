...
\begin{abstract}
This paper introduces ScholarMate, an offline-first research workspace that tackles major issues in managing academic documents and collaborative research. ScholarMate uses a hybrid setup with Flutter for cross-platform deployment and FastAPI for backend services. It allows researchers to handle PDF libraries with AI-powered semantic search, real-time collaboration, and strong annotation features while ensuring complete data ownership through Google Drive integration. The system uses a namespace-based vector isolation strategy with Pinecone for efficient retrieval-augmented generation. It also supports offline-first operations through local SQLite caching with Drift and enables real-time collaboration using Supabase Realtime. Performance tests show fast query response times, and it successfully runs on multiple platforms (Android, Web, Windows) using entirely free-tier services. The design provides full feature availability across all platforms while keeping complete control over user data and requiring no infrastructure costs for individual researchers.
\end{abstract}
...
Modern academic research relies more and more on digital document management systems. However, current solutions come with serious issues such as vendor lock-in due to proprietary storage, limited offline capability, high subscription fees, and poor support for collaborative annotation. Researchers require tools that ensure data ownership, function well without internet access, support AI-driven document analysis, and allow smooth collaboration between institutions.

ScholarMate tackles these problems with a design that blends offline-first principles with cloud-based AI features. The system keeps all user documents in their personal Google Drive accounts. This approach ensures data ownership while offering useful features like optical character recognition (OCR), text-to-speech, semantic search using RAG, and collaborative annotations.

The key contributions of this work include:

\begin{itemize}
\item A hybrid offline-first architecture enabling full functionality without internet connectivity while synchronizing seamlessly when online
...
Recent advances in RAG systems \cite{b4} demonstrate improved accuracy for question-answering tasks by grounding large language model responses in retrieved context. However, existing implementations typically assume cloud-only deployment and do not address multi-tenant isolation or offline operation requirements.

\subsection{Real-Time Collaboration}

Operational Transformation (OT) \cite{b5} and Conflict-free Replicated Data Types (CRDTs) \cite{b6} provide theoretical foundations for real-time collaboration. Commercial implementations like Google Docs achieve low-latency synchronization but require proprietary infrastructure. ScholarMate adapts these concepts for document annotation workflows using open-source components.

\section{System Architecture}

\subsection{Overview}

ScholarMate employs a three-tier architecture (Fig. 1) consisting of: (1) cross-platform Flutter frontend with local Drift database, (2) FastAPI backend providing AI orchestration and OCR services, and (3) cloud infrastructure layer including Supabase PostgreSQL for metadata, Pinecone for vector storage, and Google Drive for document storage.

\subsection{Offline-First Design}

The offline-first architecture implements a local-first data model where all operations execute against a local SQLite database (Drift) with automatic background synchronization. The system maintains three data structures:
...
When network connectivity is unavailable, operations execute optimistically against the local cache and enqueue for later synchronization. Upon reconnection, the sync manager processes queued operations using a last-write-wins conflict resolution strategy with full history preservation.
...
\textbf{Provider Pattern}: Dependency injection and state management using Flutter Provider package ensures reactive UI updates and proper resource lifecycle management.
...
\textbf{RAG Indexer}: Extracts text from PDFs using PyPDF2, chunks content using recursive character splitting (1000-character chunks with 200-character overlap), and generates embeddings using the BAAI/bge-small-en-v1.5 model via API-based inference. This model produces 384-dimensional embeddings with significantly improved semantic alignment compared to MiniLM-class models, especially for dense retrieval tasks. All chunk embeddings, along with metadata, are stored in Pinecone for efficient vector search.
...
Each vector stores comprehensive metadata:
...
% \textbf{Presence Tracking}: WebSocket-based presence system tracks active users per document, broadcasts cursor positions and page numbers, implements automatic cleanup after 30 seconds of inactivity.
...
\textbf{Core Tables}: users, files, folders, annotations, tags, file\_tags

\textbf{Collaboration Tables}: shares, collaboration\_sessions, file\_chat\_messages
...
\item All communication over HTTPS/WSS
...
\textbf{Google Drive}: 15GB storage per user (free tier)
...
\item Ask questions about their research corpus via RAG
\item Work offline during travel or in low-connectivity environments
\item Maintain complete data ownership in personal Google Drive
\end{itemize}
...
\item Annotate shared documents with real-time synchronization
...
\item Federated learning for privacy-preserving model fine-tuning
\end{itemize}
